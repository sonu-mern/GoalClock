...
function createLanguageOptions() {
  ...
  languageFlags.forEach((lang, index, arr) => {
      ...
      flag.innerText = lang.flag; // Unicode emoji

      // Fallback to SVG/PNG if Unicode emoji is not supported
      const flagImage = document.createElement('img');
      flagImage.src = `flags/${lang.code}.svg`; // Path to your flag images
      flagImage.alt = lang.name;
      flagImage.classList.add('flag-icon-fallback');
      flagImage.style.display = 'none'; // Hide by default

      // Check if Unicode emoji is supported
      const isEmojiSupported = /\p{Emoji}/u.test(lang.flag);
      if (!isEmojiSupported) {
      ...
      radioWrapper.appendChild(flagImage);
